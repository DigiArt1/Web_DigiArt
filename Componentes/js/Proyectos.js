// Proyectos.js - Con carga desde JSON
...
function setActiveNavLink() {
    ...
    navLinks.forEach(link => {
        if (link.textContent.trim() === 'Proyectos') {
...
function renderizarMenuCategorias() {
    ...
    if (!menuContainer || !proyectosData) return;

    menuContainer.innerHTML = '';
    
    Object.keys(proyectosData.categorias).forEach((key, index) => {
        const categoria = proyectosData.categorias[key];
...
function renderizarProyectos() {
    ...
    if (!proyectosContainer || !proyectosData) return;
